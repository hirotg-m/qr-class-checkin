...
| 429 | `RATE_LIMITED` | ログイン試行（8.2節）またはPIN試行（1.2節）のレート制限超過 |
...
**エラー**：`401 INVALID_PIN`（PIN不一致）、`404 NOT_FOUND`（classIdが存在しない）、`429 RATE_LIMITED`（同一クラス・同一IPからの失敗が続いた場合）

PINは4桁程度と桁数が少なく総当たりが現実的なため、8.2節のログイン試行レート制限と同じ仕組み（`LoginAttempts`テーブル、3分間に10回失敗で一時ロック）を`class_id`＋IP単位で適用する。指導者ログインの失敗カウントとは名前空間を分けており、互いに影響しない。PIN一致で成功した場合は失敗回数をリセットする。
